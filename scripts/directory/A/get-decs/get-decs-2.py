...
# Matches any color token found WITHIN a value string
COLOR_TOKEN_RE = re.compile(
    r'(#[0-9a-fA-F]{3,8}'
    r'|rgba?\s*\([^)]+\)'
    r'|hsla?\s*\([^)]+\)'
    r'|\b(black|white|red|blue|green|yellow|orange|purple|pink|'
...
def contains_color(value):
    """
    Return True if value contains ANY color token anywhere inside it.
    Catches pure colors (#fff, rgba()), compound values (1px solid #ccc),
    and gradients (linear-gradient(to bottom, #000, #fff)).
    Does NOT catch 'transparent' — that has no color variable to extract.
    """
    v = value.strip()
    if GRADIENT_RE.search(v):
        return True
    return bool(COLOR_TOKEN_RE.search(v))

def shadow_contains_color(value):
    """Check if a shadow value string contains any color token."""
    return contains_color(value)


# ── Theme detector ────────────────────────────────────────────────────
#
# Scans all files for CSS theme declarations. Supports three patterns:
#   A. [data-theme="dark"] / [data-theme="light"]
#   B. :root.dark / :root.light
#   C. @media (prefers-color-scheme: dark)
#
# Returns a dict:
#   {
#     'type':   'data-theme' | 'root-class' | 'media' | 'none',
#     'themes': {
#       'light': { '--var': 'value', ... },
#       'dark':  { '--var': 'value', ... },
#     }
...
# Patterns for each theme type
THEME_PATTERNS = {
    'data-theme': re.compile(
        r'\[data-theme=["\'](\w+)["\']\]\s*\{([^}]+)\}', re.DOTALL
    ),
    'root-class': re.compile(
        r':root\.(\w+)\s*\{([^}]+)\}', re.DOTALL
    ),
    'media': re.compile(
        r'@media\s*\(prefers-color-scheme:\s*(\w+)\)\s*\{[^{]*:root\s*\{([^}]+)\}',
        re.DOTALL
    ),
}
...
def _parse_theme_block(block_text):
    """Extract --variable: value pairs from a CSS block."""
    vars_ = {}
    for m in CSS_VAR_RE.finditer(block_text):
        var   = m.group(1).strip()
...
def detect_theme(files):
    """
    Scan all files for theme declarations.
    files: list of Path objects (all scanned source files)
    Returns theme dict as described above.
    """
    themes = {}
    theme_type = 'none'
    ...
    for path in files:
        ...
        for t_type, pattern in THEME_PATTERNS.items():
            for match in pattern.finditer(text):
                name  = match.group(1).strip().lower()   # 'dark', 'light' etc
                block = match.group(2)
                vars_ = _parse_theme_block(block)
                if vars_:
                    if t_type != theme_type and theme_type != 'none':
                        continue   # stick with first type found
                    theme_type = t_type
                    if name not in themes:
                        themes[name] = {}
                    themes[name].update(vars_)

    # Also check plain :root for non-themed color vars
    if theme_type == 'none':
        ...
        for path in files:
            ...
            for match in root_re.finditer(text):
                vars_ = _parse_theme_block(match.group(1))
                root_vars.update(vars_)
        if root_vars:
            themes['root'] = root_vars
            theme_type = 'root'

    return {'type': theme_type, 'themes': themes}


# ── Dual pipeline builder ─────────────────────────────────────────────
#
# Splits all scanned entries into two pipeline files:
#
#   pipeline-colors-shadows.txt
#     → any entry whose value IS a color (hex, rgb, rgba, hsl, named)
#     → any shadow entry whose value block CONTAINS a color
#
#   pipeline-decs.txt
#     → everything else (non-color values, shadows with no color)
#
# Deduplication key: prop + value + source_label (all three must match)
# ─────────────────────────────────────────────────────────────────────

def build_dual_pipelines(flat_data, files):
    """
    flat_data: { filename: { "section / selector": { prop: [values] } } }
    files:     list of Path objects (for theme detection)

    Returns:
      (colors_shadows_content, colors_shadows_count,
       decs_content,           decs_count)
    """
    seen_cs   = set()
    seen_decs = set()
    cs_entries   = []
    ...
    for filename, groups in flat_data.items():
        for source_label, props in groups.items():
            for prop, values in props.items():
                ...
                for value in values:
                    value = value.strip()
                    if not prop_clean or not value:
                        continue
                    key = (prop_clean, value.lower(), source_label.strip().lower())
                    if prop_clean in SHADOW_PROPS:
                        if shadow_contains_color(value):
                            if key not in seen_cs:
                                seen_cs.add(key)
                                cs_entries.append((prop_clean, value, source_label.strip()))
                        else:
                            if key not in seen_decs:
                                seen_decs.add(key)
                                decs_entries.append((prop_clean, value, source_label.strip()))
                    elif contains_color(value):
                        if key not in seen_cs:
                            seen_cs.add(key)
                            cs_entries.append((prop_clean, value, source_label.strip()))
                    else:
    ...
    cs_entries.sort(key=lambda e:   (e[2], e[0], e[1]))
    decs_entries.sort(key=lambda e: (e[0], e[2], e[1]))

    # Detect theme from source files
    theme = detect_theme(files)

    return (
        _format_pipeline(cs_entries,   "PIPELINE — COLORS + SHADOWS — for generate_colors.py", theme),
        len(cs_entries),
        _format_pipeline(decs_entries, "PIPELINE — DECS — for generate_decs.py", None),
        len(decs_entries),
    )


def _format_pipeline(entries, title, theme=None):
    if not entries:
        return "  (no entries)\n"
    ...
    # Theme block — only written for colors-shadows pipeline
    if theme is not None:
        lines.append("")
        lines.append(f"  {'─'*68}")
        lines.append(f"  THEME: {theme['type']}")
        lines.append(f"  {'─'*68}")
        if theme['themes']:
            for theme_name, vars_ in theme['themes'].items():
                lines.append(f"  ── {theme_name}")
                for var, value in vars_.items():
                    lines.append(f"  {var}: {value};")
        else:
            lines.append("  (no theme variables found)")
...
def run():
    ...
    # ── Auto-write both pipeline files
    cs_content, cs_count, decs_content, decs_count = build_dual_pipelines(flat, files)

    cs_path   = output_dir / f"pipeline-colors-shadows_{timestamp}.txt"
    decs_path = output_dir / f"pipeline-decs_{timestamp}.txt"

    cs_path.write_text(cs_content,   encoding="utf-8")
    decs_path.write_text(decs_content, encoding="utf-8")

    print(f"\n  ✓ pipeline-colors-shadows → {cs_path}")
    print(f"    {cs_count} entries  (hex/rgb/rgba/named colors + colored shadows)")
    print(f"\n  ✓ pipeline-decs          → {decs_path}")
    print(f"    {decs_count} entries  (all non-color values + uncolored shadows)")
    print()
